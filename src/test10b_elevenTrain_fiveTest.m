function test10b_elevenTrain_fiveTest()
% test10b_elevenTrain_fiveTest
%
% This script trains a speaker recognition model using "eleven" data
% from 23 students, then tests that model on "five" data 
% from those same 23 students.
%
% Directory structure (23 speakers: s1..s23):
%   D:\Program Files\Polyspace\R2021a\bin\EEC201\Eleven_Training\s{i}.wav
%   D:\Program Files\Polyspace\R2021a\bin\EEC201\Five_Test\s{i}.wav
%
% Steps:
%   1) Train codebooks for 23 speakers => from Eleven_Training\s{i}.wav
%   2) Test on Five_Test\s{i}.wav => see if it can identify speaker i
%   3) Print final accuracy => "trained on eleven, tested on five."

    clear; clc; close all;

    nSpeakers= 23; 
    baseTrain= 'D:\Program Files\Polyspace\R2021a\bin\EEC201\Eleven_Training\';
    baseTest = 'D:\Program Files\Polyspace\R2021a\bin\EEC201\Five_Test\';

    % 1) Build trainList => i=1..23 => each is an "eleven" training wave
    ...
    disp('--- Training speaker models on "eleven" => s1..s23 ---');
    speakerModels= train_speakers_demo(trainList, numFilters, numCoeffs, codebookSize);

    % 2) Build testList => from Five_Test\s{i}.wav => ID=i
    ...
    % 3) Evaluate => tries to identify speaker from "five" test
    disp('--- Testing: (Trained on eleven) => now test on "five" data ---');
    [acc, ~] = test_speakers_demo(testList, speakerModels, numFilters, numCoeffs);

    fprintf('\nFinal speaker recognition accuracy (trained=eleven, tested=five): %.2f%%\n', acc);
    disp('Done test10b_elevenTrain_fiveTest.');
end
